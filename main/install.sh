...
# VulScan Linux Installer v4.0.0
...
install_vulscan() {
    ...
    if curl -L -A "VulScan-Linux-Installer/4.0.0" -o "main.go" "$source_url"; then
...
# Generated by Linux Installer v4.0.0
...
show_usage() {
    echo "VulScan Linux Installer v4.0.0"
    echo ""
...
main() {
    ...
    if [[ -z "$VERSION" ]]; then
        if [[ "$QUIET" == false ]]; then
            ...
            echo ""
            echo -e "${YELLOW}[2] Development Release (v4.0.0-dev) - Geliştirme sürümü${NC}"
